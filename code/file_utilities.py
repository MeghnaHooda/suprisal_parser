...
def parse_dp(dp_file, wts_file, surp_file, k, ofile, num_sent=None):
	...
	with open(dp_file, 'r', encoding="utf8") as fr:
		with open(surp_file, 'w', encoding="utf8") as fw:
			with open(ofile, 'w', encoding="utf8") as fo:
				sentence_index = 0
...
def save_whole(ofile, logs, labelled, feature_set):
	...
	with open ('meta_' + ofile, 'w', encoding="utf8") as fw:
		fw.write(labelled + '\n')
...
def save_parts(ofile, n, logs, sentences):
	...
	for i in range(n):
		...
		with open(str(i) + '_train_' + ofile, 'w', encoding="utf8") as fw:
			for ind in train[i]:
			...
			for tr in range(len(ArcEagerState.transition_types)):
				fw.write(str(tr) + ' DUMMY_FEAT\n')
		with open(str(i) + '_test_' + ofile, 'w', encoding="utf8") as fw:
			for ind in test[i]:
				for trans, state in logs[ind]:
					fw.write(ArcEagerState.transition_codes[trans])
					fw.write(' ' + state + '\n')
		with open(str(i) + '_dp_' + ofile, 'w', encoding="utf8") as fw:
			for ind in test[i]:
